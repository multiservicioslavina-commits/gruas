...
const WA_PHONE_ID  = Deno.env.get("RITA_PHONE_ID") || Deno.env.get("WHATSAPP_PHONE_ID") || "3234846550";
  ...
  for (const r of riders) {
    ...
    if (r.soat_vence) {
      ...
      if (shouldNotify && !alreadyNotified) {
        ...
        if (diffDays === 1) {
          infoVencimiento = `¡MAÑANA vence el SOAT de tu ${moto}! Sin él: multa hasta $900k más inmovilización. Renúevalo HOY.`;
        } else if (diffDays <= 3) {
        ...
        const ok = await sendTemplate(tel, "recordatorio_soat_v2", [nombre, infoVencimiento]);
        if (ok) {
