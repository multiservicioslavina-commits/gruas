...
// RIDERA — registrar-almacen
// Setup + recuperación de clave por WhatsApp para cuentas de almacén.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (data.action === "estado_plantilla_recuperar" && data.setup_key === "ridera_setup_2026") {
      const res = await fetch(`${GRAPH}/${wabaId}/message_templates?fields=name,status,category,language,rejected_reason&name=codigo_clave_almacen`, {
    ...
    if (data.action === "crear_plantilla_aprobado" && data.setup_key === "ridera_setup_2026") {
      const res = await fetch(`${GRAPH}/${wabaId}/message_templates`, {
        ...
        headers: { Authorization: `Bearer ${waToken}`, "Content-Type": "application/json" },
        body: JSON.stringify({
          name: "almacen_aprobado",
          category: "UTILITY",
          language: "es_CO",
          components: [
            {
              type: "BODY",
              text: "Hola {{1}}, tu almacén ya está aprobado en Ridera. Ya puedes entrar a tu panel y cargar tu catálogo: https://gruas.ridera.com.co/mi-almacen.html",
    ...
    if (data.action === "estado_plantilla_aprobado" && data.setup_key === "ridera_setup_2026") {
      const res = await fetch(`${GRAPH}/${wabaId}/message_templates?fields=name,status,category,language,rejected_reason&name=almacen_aprobado`, {
        headers: { Authorization: `Bearer ${waToken}` },
      });
      const result = await res.json();
      return json({ ok: res.ok, status: res.status, result });
    ...
    if (data.action === "recuperar_clave_telefono") {
      ...
      const { data: almacen } = await supabase.from("almacenes")
        .select("id, nombre, telefono, auth_id").eq("telefono", telefono).maybeSingle();

      if (almacen?.auth_id) {
        const code = String(Math.floor(100000 + Math.random() * 900000));
        await supabase.from("almacen_reset_codes").insert({
          telefono,
          code,
          expires_at: new Date(Date.now() + 15 * 60 * 1000).toISOString(),
        });
        await sendWATemplate(almacen.telefono, "codigo_clave_almacen", "es_CO", [code]);
      }
    ...
    if (data.action === "confirmar_reset_telefono") {
      ...
      const { data: reset } = await supabase.from("almacen_reset_codes")
      ...
      if (!almacen?.auth_id) return json({ ok: false, error: "Cuenta no encontrada" }, 404);

      const { error: updErr } = await supabase.auth.admin.updateUserById(almacen.auth_id, { password: newPassword });
      if (updErr) return json({ ok: false, error: updErr.message }, 500);

      await supabase.from("almacen_reset_codes").update({ used: true }).eq("id", reset.id);
      return json({ ok: true });
    }
