...
${bloqueEstilo}${bloqueContextoRider}${bloqueContextoAlertas}${bloqueContextoLegal}${bloqueContextoSocial}${bloqueContextoNavigador}${bloqueContextoAcademia}${bloqueContextoAnalytics}
...
✅ Analytics: Estadísticas personales, patrones de conducción, benchmarks comunitarios
...
async function responder(
  ...
  const system = buildSystemPrompt(consentimiento, conVoz, nombreRider, contextoRider, contextoAlertas, contextoLegal, contextoSocial, contextoNavigador, contextoAcademia, contextoAnalytics);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const [history, consentimiento, nombreRider, contextoRider, contextoAlertas, contextoLegal, contextoSocial, contextoNavigador, contextoAcademia, contextoAnalytics] = await Promise.all([
      getHistory(from, 10),
    ...
    try {
      ...
      reply = Deno.env.get("RITA_ORQUESTADOR") !== "false"
        ? await responderOrquestado(message, history, from, consentimiento, conVoz, nombreRider, contextoRider, contextoAlertas, contextoLegal, contextoSocial, contextoNavigador, contextoAcademia, contextoAnalytics)
        : await responder(message, history, from, consentimiento, conVoz, nombreRider, contextoRider, contextoAlertas, contextoLegal, contextoSocial, contextoNavigador, contextoAcademia, contextoAnalytics);
    } catch (e) {
