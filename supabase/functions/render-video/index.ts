...
Deno.serve(async (req: Request) => {
  ...
  const pathname = url.pathname; // e.g. /functions/v1/render-video/status/uuid
  ...
  if (req.method === "POST") {
    try {
      ...
      // Responder con ambos campos para compatibilidad con versiones old y new
  ...
  if (req.method === "GET") {
    try {
      // Detectar jobId en path (/status/uuid) o query param (?renderId=uuid)
      let jobId: string | null = null;

      // Path-style: /render-video/status/{uuid}
      const pathMatch = pathname.match(/\/status\/([^/]+)$/);
      if (pathMatch) {
        jobId = pathMatch[1];
      }
      // Query-style: ?renderId=uuid
      if (!jobId) {
      ...
      const data = await res.json();
      // Railway: { state, progress, url, error }
